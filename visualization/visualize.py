...
def visualize_venn(train_df: pd.DataFrame, test_df:pd.DataFrame, plot_column:string) -> Tuple[plt.Figure, plt.Axes]:
    """Make venn figure.

    Args:
        train_df (pd.DataFrame): Dataframe of train data
        test_df (pd.DataFrame): Dataframe of test data
        plot_column (string): A specified categorical column for visualization.

    Returns:
        Tuple[plt.Figure, plt.Axes]: Figure & Axes
    """
    fig, ax = plt.subplots(figsize=(6, 4))
    venn2(subsets=(set(train_df[plot_column].unique()), set(test_df[plot_column].unique())), set_labels=('Train', 'Test'), color=color_map, ax=ax)
    ax.set_title(f"{plot_column}")
    fig.tight_layout()
    return fig,ax

def visualize_compare_distribution(x:List[float],y:List[float],x_label:string,y_label:string)->Tuple[plt.Figure, plt.Axes]:
    ...
    Args:
        ...
        x_label (string): x's label text
        y_label (string): y's label text
...
def visualize_importance(models:List[Any], train_columns: pd.DataFrame,top:int=50,model_type='lgbm')-> Tuple[plt.Figure, plt.Axes]:
    ...
    if model_type == 'lgbm':
        ...
        order = feature_importance_df.groupby('column')\
            ...
            .sort_values('feature_importance', ascending=False).index[:top]
...
def visualize_mulitiple_time_series(df:pd.DataFrame,x:string,y1:string,y2:string=None)-> Tuple[plt.Figure, plt.Axes]:
    """make figure of time series.(single or multiple)

    Args:
        df (pd.DataFrame): [description]
        x (string): x-axis column name
        y1 (string): y-axis column name
        y2 (string): y-axis column name
